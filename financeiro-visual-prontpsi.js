...
(function () {
    ...
    // ── Buscar e processar dados (todas as fontes) ─────────────────
    async function buscarTodos() {
        // Busca todas as fontes em paralelo
        const anoAtual = new Date().getFullYear();
        const [resOnline, resManuais] = await Promise.all([
            fetch(`${API_URL}/api/agendamentos-online`, { headers: headersAuth() }),
            fetch(`${API_URL}/api/financeiro/relatorio-anual?ano=${anoAtual}`, { headers: headersAuth() })
        ]);

        const online = resOnline.ok ? await resOnline.json() : [];
        const relatorio = resManuais.ok ? await resManuais.json() : { meses: [], totalAnual: 0 };

        return { online, relatorio };
    }
    ...
    async function renderDashboard(ano) {
        ...
        try {
            const { online, relatorio } = await buscarTodos();
            const confirmados = online.filter(a => a.status === 'confirmado');

            const agora = new Date();
            const mesAtual = agora.getMonth(); // 0-11
            const anoAtual = agora.getFullYear();

            // ── Dados por mês combinando ONLINE + MANUAIS + PACOTES ──
            ...
            // Manuais e pacotes do relatório anual (se for o ano atual)
            if (ano === anoAtual && relatorio.meses) {
                relatorio.meses.forEach(m => {
                    porMes[m.mes - 1] += (m.manuais || 0) + (m.pacotes || 0);
                });
            }
            ...
            // ── Consultas do mês atual (online + manuais) ──
            const consultasMesOnline = online.filter(a => {
                const d = new Date(a.data_consulta + 'T00:00:00');
                return d.getMonth() === mesAtual && d.getFullYear() === anoAtual;
            });
            const confirmMes = consultasMesOnline.filter(a => a.status === 'confirmado').length;
            const ticketMedio = valorMesAtual > 0 && confirmMes > 0 ? valorMesAtual / confirmMes : 0;
            ...
            // ── Últimos 6 meses (tendência combinada) ──
            const ultimos6 = [];
            for (let i = 5; i >= 0; i--) {
                const d = new Date(anoAtual, mesAtual - i, 1);
                const m = d.getMonth();
                const a = d.getFullYear();
                let total = confirmados
                    .filter(ag => {
                        const dd = new Date(ag.data_consulta + 'T00:00:00');
                        return dd.getMonth() === m && dd.getFullYear() === a;
                    })
                    .reduce((s, ag) => s + parseFloat(ag.valor || 0), 0);
                // Adiciona manuais e pacotes se for ano atual
                if (a === anoAtual && relatorio.meses) {
                    const mesRel = relatorio.meses.find(mr => mr.mes === m + 1);
                    if (mesRel) total += (mesRel.manuais || 0) + (mesRel.pacotes || 0);
                }
